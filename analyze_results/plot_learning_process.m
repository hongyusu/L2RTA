        ...
        for gi = 1:length(gs)
            ...
            for ci = 1:length(cs)
                ...
                for ti = 1:length(ts)
                    ...
                    system(sprintf('for i in 1 4 8 12 16; do cat  ../outputs/backup_run/%s_tree_%d_f1_l2_k${i}_c%s_s%s_n%s_RSTAs.log|tail -n6|head -n4|awk -F%s %s %s{print $6%s %s$9%s %s$11%s %s$13%s %s$14%s %s$32%s %s$33}%s|sed s/\\%%//g; done > tmp',...
                        dname,t,c,g,n,qs,qs,qs,qd,qd,qd,qd,qd,qd,qd,qd,qd,qd,qd,qd,qs));
                    x = dlmread('tmp');
                    
                    x = mean(reshape(x,4,size(x,2)*length(ks)),1);
