    ...
    for di = 1:length(dataset)
        ...
        for ic = 1:length(cs)
            ...
            for i = 1:length(ts)
                ...
                system(sprintf('for i in 1 2 4 6 8 10; do cat  ../outputs/backup_run/%s_tree_%d_f1_l2_k${i}_c%d_s1_n1_RSTAs.log|tail -n3|head -n1|awk -F%s %s %s{print $6%s %s$9%s %s$11%s %s$13%s %s$14%s %s$32%s %s$33}%s|sed s/\\%%//g; done > tmp',...
                    dname,t,c,qs,qs,qs,qd,qd,qd,qd,qd,qd,qd,qd,qd,qd,qd,qd,qs));
                x = dlmread('tmp')
                % training_1_err, training_ham_err,test_1_err, test_ham_err, yi_tr, yi_ts
                system(sprintf('for i in 1 2 4 6 8 10; do cat  ../outputs/backup_run/%s_tree_%d_f1_l2_k${i}_c%d_s1_n1_RSTAs.log|tail -n2|head -n1|awk -F%s %s %s{print $4%s %s$7%s %s$10%s %s$13%s %s$18%s %s$24}%s|sed s/\\%%//g; done > tmp',...
                    dname,t,c,qs,qs,qs,qd,qd,qd,qd,qd,qd,qd,qd,qd,qd,qs));
                y = dlmread('tmp');
                x = [ks',x,y];
        ...
        export_fig(sprintf('plot_%s.jpg',dname),figure1)
